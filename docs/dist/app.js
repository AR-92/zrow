...
`)}function F(e,a,n,o={}){let t=document.getElementById(e),r=document.getElementById(a);if(!t||!r)return;let{min:s=60,max:i=1/0,onResize:d}=o,p,u;function b(f){f.preventDefault(),p=n==="h"?f.clientX:f.clientY,u=n==="h"?r.offsetWidth:r.offsetHeight,r.classList.add("no-transition"),document.body.style.cursor=n==="h"?"col-resize":"row-resize",document.body.style.userSelect="none",document.addEventListener("mousemove",y),document.addEventListener("mouseup",x)}function y(f){let h=n==="h"?f.clientX-p:f.clientY-p,R=u+h;R=Math.max(s,Math.min(i,R)),n==="h"?r.style.width=R+"px":r.style.height=R+"px"}function x(){r.classList.remove("no-transition"),document.body.style.cursor="",document.body.style.userSelect="",document.removeEventListener("mousemove",y),document.removeEventListener("mouseup",x),d&&d(n==="h"?r.offsetWidth:r.offsetHeight)}t.addEventListener("mousedown",b)}document.addEventListener("DOMContentLoaded",()=>{let{theme:e}=l.get();document.documentElement.classList.toggle("dark",e==="dark"),m(),l.subscribe(()=>{let a=l.get(),n=c("#status-text");n&&(n.textContent=a.statusText);let o=c("#status-icon");o&&(o.className=`w-2 h-2 rounded-full ${a.queryRunning?"bg-amber-400 animate-pulse":a.activeConnectionId?"bg-emerald-400":"bg-gray-600"}`)}),F("sidebar-drag","sidebar","h",{min:120,max:500,onResize:a=>T.setSidebarWidth(a)}),c("#sidebar-toggle")?.addEventListener("click",()=>{T.toggleSidebar(),m()})});function m(){Se(),$e(),Oe(),je(),v()}function Se(){let e=c("#sidebar"),a=l.get(),n=a.sidebarCollapsed;e.style.width=n?"0px":a.sidebarWidth+"px";let o=c("#sidebar-toggle");o&&(o.innerHTML=`<i data-lucide="${n?"panel-left-open":"panel-left-close"}" class="w-3 h-3"></i>`),A(e);let{connections:t,activeConnectionId:r,tables:s,currentTable:i}=a;e.innerHTML=`
  ...
  `,document.addEventListener("contextmenu",u=>{let b=c("#table-context-menu");b&&b.remove()});let d=c("#sidebar-body");if(!t.length&&!r){d.innerHTML=`
    ...
    `,c("#btn-new-conn")?.addEventListener("click",oe),v(d);return}let p="";for(let u of t){let b=u.id===r;if(p+=`
    ...
    `,c("#btn-new-query")?.addEventListener("click",()=>{l.setKey("currentTable",null),l.setKey("currentTableData",null),l.setKey("currentTableInfo",null),T.addTab("editor"),m()}),v(e);return}let t=n.find(r=>r.id===o);t?.type==="editor"&&De(e,t,a)}function De(e,a){let n=l.get(),o=n.resultsPanelHeight;e.innerHTML=`
...
`.repeat(Math.max(1,(u.match(/\n/g)||"").length+1)),s.textContent=ne(u),T.updateTabSQL(a.id,u)}t.addEventListener("input",d),t.addEventListener("scroll",()=>{r.scrollTop=t.scrollTop,r.scrollLeft=t.scrollLeft,s.scrollTop=t.scrollTop}),t.addEventListener("keydown",u=>{if(u.key==="Tab"){u.preventDefault();let b=t.selectionStart;t.value=t.value.substring(0,b)+"  "+t.value.substring(t.selectionEnd),t.selectionStart=t.selectionEnd=b+2,d()}(u.ctrlKey||u.metaKey)&&u.key==="Enter"&&(u.preventDefault(),re())}),d(),setTimeout(()=>t.focus(),50),c("#btn-run")?.addEventListener("click",re),ae(n);let p=l.subscribe(()=>{ae(l.get()),Ue(c("#results-panel"))},!1);F("results-drag","results-panel","v",{min:60,max:window.innerHeight*.8,onResize:u=>T.setResultsPanelHeight(u)})}function ae(e){let a=c("#editor-status");a&&(e.queryRunning?a.innerHTML='<i data-lucide="loader-circle" class="w-3 h-3 animate-spin inline-block mr-1"></i> Running...':e.queryError?a.innerHTML=`<i data-lucide="alert-circle" class="w-3 h-3 inline-block mr-1"></i> ${e.queryError}`:e.results?a.textContent=`${e.results.rows.length} rows in ${e.results.duration}ms`:a.textContent="Ready",v(a?.parentElement))}function Ue(e){if(!e)return;let a=l.get();if(a.queryRunning){e.innerHTML='<div class="flex items-center justify-center h-full gap-2 text-sm text-gray-500"><i data-lucide="loader-circle" class="w-4 h-4 animate-spin"></i> Running...</div>',v(e);return}if(a.queryError){e.innerHTML=`<div class="flex items-start gap-2 p-4 text-sm text-red-400"><i data-lucide="alert-circle" class="w-4 h-4 mt-0.5 shrink-0"></i> ${a.queryError}</div>`,v(e);return}if(!a.results?.columns?.length){e.innerHTML='<div class="flex flex-col items-center justify-center h-full gap-3 text-gray-600"><i data-lucide="arrow-up-right" class="w-8 h-8 opacity-30"></i><span class="text-xs">Run a query to see results</span></div>',v(e);return}e.innerHTML=le(a.results,!0),v(e)}async function re(){let e=c("#editor-input")?.value?.trim();if(!(!e||!l.get().activeConnectionId))try{await q(e)}catch{}}function Me(e,a){let n=a.currentTableData,o=a.currentTableInfo,t=a.currentTable;if(!n){e.innerHTML='<div class="flex items-center justify-center h-full text-gray-600 text-sm">Loading...</div>';return}let r=o?.columns?.find(d=>d.primaryKey)?.name||null,s=a.schemaPanelCollapsed,i=o?.columns?.map((d,p)=>`<div class="flex items-center gap-2 px-3 py-1.5 text-xs border-b border-gray-800/30 hover:bg-gray-800/20 group">
  ...
  `,c("#btn-new-tab")?.addEventListener("click",()=>{T.addTab("editor"),m()}),c("#btn-new-table-footer")?.addEventListener("click",()=>ie()),c("#btn-theme")?.addEventListener("click",T.toggleTheme),v(e)}function oe(e){let a=c("#modal-overlay");a.classList.remove("hidden");let n=c("#modal-content");A(n);let o=e?.database&&e.database!==":memory:";n.innerHTML=`
