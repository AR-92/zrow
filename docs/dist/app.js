...
`;function G(e,n){let t=le.split(";").filter(o=>o.trim());for(let o of t)try{n(o+";")}catch(a){console.warn("Seed statement failed (likely already exists):",a.message)}}function H(){return new Promise((e,n)=>{let t=indexedDB.open("zrow_dbs",1);t.onupgradeneeded=o=>{let a=o.target.result;a.objectStoreNames.contains("dbs")||a.createObjectStore("dbs",{keyPath:"id"})},t.onsuccess=o=>e(o.target.result),t.onerror=()=>n(new Error("Failed to open IndexedDB"))})}async function ce(e){let n=await H();return new Promise((t,o)=>{let r=n.transaction("dbs","readonly").objectStore("dbs").get(e);r.onsuccess=()=>{n.close(),t(r.result?.data||null)},r.onerror=()=>{n.close(),o(new Error("Failed to read DB"))}})}async function de(e,n){let t=await H();return new Promise((o,a)=>{let r=t.transaction("dbs","readwrite");r.objectStore("dbs").put({id:e,data:n,updated:Date.now()}),r.oncomplete=()=>{t.close(),o()},r.onerror=()=>{t.close(),a()}})}var k=class{constructor(){this._db=null,this._SQL=null,this._name=null}async open(n,t){if(this._name=n,this._SQL=await window.initSqlJs({locateFile:o=>(t||"dist/vendor/")+o}),this._db=new this._SQL.Database,n&&n!==":memory:")try{let o=await ce(n);o&&(this._db=new this._SQL.Database(o))}catch{}return this}seedIfEmpty(){try{let n=this._db.exec("SELECT count(*) as cnt FROM sqlite_master WHERE type='table'");if(n.length&&n[0].values[0][0]>0)return}catch{}G(this._db,n=>this._db.exec(n))}async save(){if(this._name&&this._name!==":memory:"&&this._db)try{await de(this._name,this._db.export())}catch{}}exec(n){let t=performance.now(),o=this._db.exec(n),a=[],r=[],s=this._db.getRowsModified();for(let u of o)u.columns?.length&&(a=u.columns.map(d=>({name:d,type:"text"})),r=u.values.map(d=>{let p={};return u.columns.forEach((b,f)=>{p[b]=d[f]}),p}));return/^\s*(INSERT|UPDATE|DELETE|CREATE|DROP|ALTER)/i.test(n)&&this.save(),{columns:a,rows:r,affectedRows:s,duration:Math.round(performance.now()-t)}}getTables(){let n=this._db.exec("SELECT name FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%' ORDER BY name");return n.length?n[0].values.map(t=>{let o=t[0],a=this._db.exec(`PRAGMA table_info("${o}")`),r=a.length?a[0].values.map(s=>({name:s[1],type:s[2],nullable:!s[3],defaultValue:s[4],primaryKey:!!s[5]})):[];return{name:o,type:"table",columns:r}}):[]}_escapeId(n){return`"${n.replace(/"/g,'""')}"`}_escapeLike(n){return n.replace(/'/g,"''").replace(/%/g,"\\%").replace(/_/g,"\\_")}getTableData(n,{limit:t=200,offset:o=0,filters:a={},sort:r=null}={}){try{let s="",i=[];for(let[x,L]of Object.entries(a)){if(L===""||L==null)continue;let R=this._escapeId(x);L==="__NULL__"?i.push(`${R} IS NULL`):L==="__NOTNULL__"?i.push(`${R} IS NOT NULL`):i.push(`${R} LIKE '%${this._escapeLike(L)}%' ESCAPE '\\'`)}i.length&&(s=" WHERE "+i.join(" AND "));let u="";if(r&&r.column){let x=r.direction==="desc"?"DESC":"ASC";u=` ORDER BY ${this._escapeId(r.column)} ${x}`}let d=`SELECT * FROM ${this._escapeId(n)}${s}${u} LIMIT ${t} OFFSET ${o}`,p=this._db.exec(d),b=this._db.exec(`SELECT COUNT(*) as cnt FROM ${this._escapeId(n)}${s}`),f=b.length?b[0].values[0][0]:0;if(!p.length)return{columns:[],rows:[],total:0};let T=p[0].columns.map(x=>({name:x,type:"text"})),N=p[0].values.map(x=>{let L={};return p[0].columns.forEach((R,$)=>{L[R]=x[$]}),L});return{columns:T,rows:N,total:f}}catch{return{columns:[],rows:[],total:0}}}updateRow(n,t,o){let a=Object.entries(t).filter(([r])=>r!=="id").map(([r,s])=>`"${r}" = ${s===null?"NULL":typeof s=="string"?`'${s.replace(/'/g,"''")}'`:s}`).join(", ");a&&this.exec(`UPDATE "${n}" SET ${a} WHERE id = ${typeof o=="string"&&isNaN(Number(o))?`'${o}'`:o}`)}deleteRow(n,t){this.exec(`DELETE FROM "${n}" WHERE id = ${typeof t=="string"&&isNaN(Number(t))?`'${t}'`:t}`)}updateRowByPk(n,t,o,a){let r=Object.entries(t).filter(([s])=>s!==o).map(([s,i])=>`"${s}" = ${i===null?"NULL":typeof i=="string"?`'${i.replace(/'/g,"''")}'`:i}`).join(", ");r&&this.exec(`UPDATE "${n}" SET ${r} WHERE "${o}" = ${typeof a=="string"&&isNaN(Number(a))?`'${a.replace(/'/g,"''")}'`:a}`)}deleteRowByPk(n,t,o){this.exec(`DELETE FROM "${n}" WHERE "${t}" = ${typeof o=="string"&&isNaN(Number(o))?`'${o.replace(/'/g,"''")}'`:o}`)}insertRow(n,t){let o=Object.keys(t).map(r=>`"${r}"`),a=Object.values(t).map(r=>r==null?"NULL":typeof r=="string"?`'${r.replace(/'/g,"''")}'`:r);this.exec(`INSERT INTO "${n}" (${o.join(", ")}) VALUES (${a.join(", ")})`)}createTable(n,t){let o=t.map(a=>{let r=`"${a.name}" ${a.type}`;return a.primaryKey&&(r+=" PRIMARY KEY"),a.autoIncrement&&(r+=" AUTOINCREMENT"),a.notNull&&(r+=" NOT NULL"),a.defaultValue!=null&&a.defaultValue!==""&&(r+=` DEFAULT ${typeof a.defaultValue=="string"?`'${a.defaultValue}'`:a.defaultValue}`),a.unique&&(r+=" UNIQUE"),r});this.exec(`CREATE TABLE "${n}" (${o.join(", ")})`)}dropTable(n){this.exec(`DROP TABLE IF EXISTS "${n}"`)}addColumn(n,t){let o=`"${t.name}" ${t.type}`;t.notNull&&(o+=" NOT NULL"),t.defaultValue!=null&&t.defaultValue!==""&&(o+=` DEFAULT ${typeof t.defaultValue=="string"?`'${t.defaultValue}'`:t.defaultValue}`),this.exec(`ALTER TABLE "${n}" ADD COLUMN ${o}`)}dropColumn(n,t){this.exec(`ALTER TABLE "${n}" DROP COLUMN "${t}"`)}renameTable(n,t){this.exec(`ALTER TABLE "${n}" RENAME TO "${t}"`)}getTableInfo(n){let t=this._db.exec(`PRAGMA table_info("${n}")`),o=this._db.exec(`PRAGMA index_list("${n}")`),a=this._db.exec(`PRAGMA foreign_key_list("${n}")`);return{columns:t.length?t[0].values.map(r=>({cid:r[0],name:r[1],type:r[2],notNull:!!r[3],defaultValue:r[4],primaryKey:!!r[5]})):[],indexes:o.length?o[0].values.map(r=>({name:r[1],unique:!r[2],origin:r[3]})):[],foreignKeys:a.length?a[0].values.map(r=>({column:r[3],refTable:r[2],refColumn:r[4]})):[]}}};function ue(){try{return JSON.parse(localStorage.getItem("zrow_connections")||"[]")}catch{return[]}}function j(e){localStorage.setItem("zrow_connections",JSON.stringify(e))}var pe=Date.now();function be(){return++pe}var ye={theme:localStorage.getItem("zrow_theme")||"dark",connections:ue(),activeConnectionId:null,tabs:[],activeTabId:null,statusText:"Ready",results:null,queryRunning:!1,queryError:null,tables:[],currentTable:null,currentTableData:null,currentTableInfo:null,sidebarView:"tables",recordCount:null,tableFilters:{},tableSort:null},l=P(ye),w={toggleTheme(){let e=l.get().theme==="dark"?"light":"dark";l.setKey("theme",e),localStorage.setItem("zrow_theme",e),document.documentElement.classList.toggle("dark",e==="dark")},addConnection(e){let{connections:n}=l.get(),t=e.id||"conn_"+Date.now(),o=e.id?n.map(a=>a.id===e.id?{...e}:a):[...n,{...e,id:t}];return l.setKey("connections",o),j(o),t},deleteConnection(e){let{connections:n,activeConnectionId:t,tabs:o}=l.get(),a=n.filter(r=>r.id!==e);l.setKey("connections",a),j(a),l.setKey("tabs",o.filter(r=>r.connectionId!==e)),t===e&&(l.setKey("activeConnectionId",null),l.setKey("tables",[]),l.setKey("currentTable",null),l.setKey("currentTableData",null))},setActiveConnection(e){l.setKey("activeConnectionId",e)},addTab(e="editor",n={}){let{tabs:t,activeConnectionId:o}=l.get(),a=be(),r={id:a,type:e,name:n.name||(e==="editor"?`Query ${t.filter(s=>s.type==="editor").length+1}`:n.tableName||"Table"),connectionId:n.connectionId||o,sql:n.sql||"",tableName:n.tableName||null};return l.setKey("tabs",[...t,r]),l.setKey("activeTabId",a),a},closeTab(e){let{tabs:n,activeTabId:t}=l.get(),o=n.findIndex(r=>r.id===e),a=n.filter(r=>r.id!==e);if(l.setKey("tabs",a),t===e){let r=Math.min(o,a.length-1);l.setKey("activeTabId",a.length?a[Math.max(0,r)].id:null)}},setActiveTab(e){l.setKey("activeTabId",e)},updateTabSQL(e,n){l.setKey("tabs",l.get().tabs.map(t=>t.id===e?{...t,sql:n}:t))},setStatus(e){l.setKey("statusText",e)},setResults(e){l.setKey("results",e),l.setKey("queryError",null)},setQueryError(e){l.setKey("queryError",e),l.setKey("results",null)},setQueryRunning(e){l.setKey("queryRunning",e)},setTables(e){l.setKey("tables",e)},setCurrentTable(e){l.setKey("currentTable",e)},setCurrentTableData(e){l.setKey("currentTableData",e)},setCurrentTableInfo(e){l.setKey("currentTableInfo",e)},setSidebarView(e){l.setKey("sidebarView",e)},setRecordCount(e){l.setKey("recordCount",e)},setTableFilters(e){l.setKey("tableFilters",e)},setTableSort(e){l.setKey("tableSort",e)},clearTableFilters(){l.setKey("tableFilters",{})}};var y=null;function M(){return y}async function _(e){let n=new k;return await n.open(e.database||e.name,e.wasmPath),e.seed!==!1&&n.seedIfEmpty(),y=n,l.setKey("activeConnectionId",e.id),l.setKey("tables",n.getTables()),l.setKey("statusText",`Connected \u2014 ${e.name}`),n}async function V(){y&&await y.save(),y=null,l.setKey("activeConnectionId",null),l.setKey("tables",[]),l.setKey("currentTable",null),l.setKey("currentTableData",null),l.setKey("currentTableInfo",null)}async function Y(e){if(!y)throw new Error("Not connected");l.setKey("queryRunning",!0),l.setKey("queryError",null);try{let n=y.exec(e);return l.setKey("results",n),l.setKey("queryRunning",!1),l.setKey("statusText",`${n.rows.length} rows in ${n.duration}ms`),n}catch(n){throw l.setKey("queryError",n.message),l.setKey("queryRunning",!1),l.setKey("statusText","Query failed"),n}}async function D(e){if(y)try{l.setKey("currentTable",e),l.setKey("tableFilters",{}),l.setKey("tableSort",null);let{tableFilters:n,tableSort:t}=l.get(),o=y.getTableData(e,{filters:n,sort:t}),a=y.getTableInfo(e);l.setKey("currentTableData",o),l.setKey("currentTableInfo",a),l.setKey("statusText",`Table "${e}" \u2014 ${o.total} rows`)}catch{}}async function S(){let{currentTable:e,tableFilters:n,tableSort:t}=l.get();if(!(!e||!y||!l.get().activeConnectionId))try{let r=y.getTableData(e,{filters:n,sort:t}),s=y.getTableInfo(e);l.setKey("currentTableData",r),l.setKey("currentTableInfo",s);let i=Object.values(n).filter(d=>d&&d!=="").length,u=[`${r.total} rows`];i&&u.push(`${i} filter${i>1?"s":""}`),t&&u.push(`sorted by ${t.column}`),l.setKey("statusText",`Table "${e}" \u2014 ${u.join(", ")}`)}catch{}}function O(){let{currentTable:e}=l.get();e&&D(e)}function I(){y&&l.setKey("tables",y.getTables())}async function K(e,n){y&&(y.insertRow(e,n),l.setKey("statusText",`Row inserted into "${e}"`),O(),I())}async function X(e,n,t,o){y&&(y.updateRowByPk(e,n,t,o),l.setKey("statusText",`Row updated in "${e}"`),O())}async function q(e,n,t){y&&(y.deleteRowByPk(e,n,t),l.setKey("statusText",`Row deleted from "${e}"`),O(),I())}async function B(e,n){y&&(y.createTable(e,n),l.setKey("statusText",`Table "${e}" created`),I())}async function W(e){y&&(y.dropTable(e),l.get().currentTable===e&&(l.setKey("currentTable",null),l.setKey("currentTableData",null),l.setKey("currentTableInfo",null)),l.setKey("statusText",`Table "${e}" dropped`),I())}async function Q(e,n){y&&(y.addColumn(e,n),l.setKey("statusText",`Column "${n.name}" added to "${e}"`),O(),I())}async function J(e,n){y&&(y.dropColumn(e,n),l.setKey("statusText",`Column "${n}" dropped from "${e}"`),O(),I())}async function z(e,n){y&&(y.renameTable(e,n),l.get().currentTable===e&&l.setKey("currentTable",n),l.setKey("statusText",`Table renamed to "${n}"`),I())}function C(e,n={},...t){let o=document.createElement(e);for(let[a,r]of Object.entries(n))a==="className"?o.className=r:a==="style"&&typeof r=="object"?Object.assign(o.style,r):a.startsWith("on")?o.addEventListener(a.slice(2).toLowerCase(),r):a==="dataset"&&typeof r=="object"?Object.assign(o.dataset,r):a==="html"?o.innerHTML=r:a==="text"?o.textContent=r:o.setAttribute(a,r);for(let a of t)a!=null&&(typeof a=="string"||typeof a=="number"?o.appendChild(document.createTextNode(a)):a instanceof Node&&o.appendChild(a));return o}function c(e,n=document){return n.querySelector(e)}function U(e,n=document){return n.querySelectorAll(e)}function v(e,n,t,o){let a=r=>{let s=r.target.closest(n);s&&e.contains(s)&&o(r,s)};return e.addEventListener(t,a),()=>e.removeEventListener(t,a)}function A(e){for(;e.firstChild;)e.removeChild(e.firstChild)}function E(e){let n=document.createElement("div");return n.textContent=e,n.innerHTML}function h(e){window.lucide&&(e?lucide.createIcons({root:e}):lucide.createIcons())}var me=new Set(["SELECT","FROM","WHERE","INSERT","INTO","VALUES","UPDATE","SET","DELETE","CREATE","TABLE","ALTER","DROP","INDEX","VIEW","JOIN","LEFT","RIGHT","INNER","OUTER","FULL","CROSS","ON","AND","OR","NOT","IN","NULL","IS","LIKE","BETWEEN","EXISTS","UNION","ALL","DISTINCT","AS","ORDER","BY","GROUP","HAVING","LIMIT","OFFSET","ASC","DESC","CASE","WHEN","THEN","ELSE","END","WITH","RECURSIVE","PRIMARY","KEY","FOREIGN","REFERENCES","CASCADE","CONSTRAINT","DEFAULT","CHECK","UNIQUE","IF","GRANT","REVOKE","COMMIT","ROLLBACK","BEGIN","TRANSACTION","EXPLAIN","ANALYZE","TRUNCATE","RETURNING","USING","NATURAL","EXCEPT","INTERSECT","FETCH","NEXT","ROWS","ONLY","FOR","OF","MERGE","MATCHED","DO","NOTHING"]),ge=new Set(["INT","INTEGER","BIGINT","SMALLINT","TINYINT","BOOLEAN","BIT","FLOAT","DOUBLE","REAL","DECIMAL","NUMERIC","MONEY","CHAR","VARCHAR","TEXT","CLOB","NCHAR","NVARCHAR","NTEXT","BINARY","VARBINARY","BLOB","BYTEA","DATE","TIME","TIMESTAMP","DATETIME","YEAR","INTERVAL","JSON","JSONB","UUID","ARRAY","ENUM","SERIAL","BIGSERIAL","GEOMETRY","GEOGRAPHY","POINT","LINESTRING","POLYGON"]),fe=new Set(["COUNT","SUM","AVG","MIN","MAX","COALESCE","NULLIF","CAST","CONVERT","SUBSTRING","UPPER","LOWER","TRIM","LENGTH","REPLACE","CONCAT","NOW","CURRENT_DATE","CURRENT_TIME","CURRENT_TIMESTAMP","EXTRACT","DATE_PART","DATE_TRUNC","ROUND","CEIL","FLOOR","ABS","MOD","POWER","SQRT","EXP","LN","LOG","GREATEST","LEAST","ROW_NUMBER","RANK","DENSE_RANK","LEAD","LAG","FIRST_VALUE","LAST_VALUE","NTH_VALUE","STRING_AGG","ARRAY_AGG","JSON_AGG","GROUP_CONCAT","LISTAGG","TO_CHAR","TO_DATE","TO_NUMBER","LEFT","RIGHT","POSITION","STRPOS","REGEXP_REPLACE","REGEXP_MATCH","SPLIT_PART","MD5","SHA256","RANDOM","GEN_RANDOM_UUID"]),xe=new Set(["TRUE","FALSE","NULL","UNKNOWN","CURRENT_USER","SESSION_USER","CURRENT_DATE","CURRENT_TIME","CURRENT_TIMESTAMP","LOCALTIME","LOCALTIMESTAMP","CURRENT_CATALOG","CURRENT_SCHEMA","DEFAULT"]);function Z(e){if(!e)return"";let n=[],t=0,o=e.length;for(;t<o;){if(e[t]===`
...
`;)a+=e[t++];n.push({type:"comment",value:"--"+a});continue}if(e[t]==="/"&&e[t+1]==="*"){let a="/*";for(t+=2;t<o-1&&!(e[t]==="*"&&e[t+1]==="/");)a+=e[t++];t<o-1&&(a+="*/",t+=2),n.push({type:"comment",value:a});continue}if(e[t]==="'"||e[t]==='"'||e[t]==="`"){let a=e[t],r=a;for(t++;t<o;){if(e[t]==="\\"&&t+1<o){r+=e[t]+e[t+1],t+=2;continue}if(r+=e[t],e[t]===a){t++;break}t++}n.push({type:"string",value:r});continue}if(e[t]==="$"&&t+1<o&&e[t+1]==="$"){let a="$$";for(t+=2;t<o-1&&!(e[t]==="$"&&e[t+1]==="$");)a+=e[t++];t<o-1&&(a+="$$",t+=2),n.push({type:"string",value:a});continue}if(/[0-9]/.test(e[t])&&(t===0||/[\s,()=<>!+\-*/%]/.test(e[t-1]))){let a="";for(;t<o&&/[0-9.]/.test(e[t]);)a+=e[t++];a.endsWith(".")&&(a=a.slice(0,-1),t--),n.push({type:"number",value:a});continue}if(/[a-zA-Z_]/.test(e[t])){let a="";for(;t<o&&/[a-zA-Z0-9_]/.test(e[t]);)a+=e[t++];let r=a.toUpperCase();me.has(r)?n.push({type:"keyword",value:a}):ge.has(r)?n.push({type:"type",value:a}):fe.has(r)?n.push({type:"function",value:a}):xe.has(r)?n.push({type:"builtin",value:a}):n.push({type:"text",value:a});continue}if(/[()]/.test(e[t])){n.push({type:"text",value:e[t]}),t++;continue}if(e[t]===":"&&t+1<o&&/[a-zA-Z]/.test(e[t+1])){let a=":";for(t++;t<o&&/[a-zA-Z0-9_]/.test(e[t]);)a+=e[t++];n.push({type:"variable",value:a});continue}if(/[=<>!+\-*/%,;.]/.test(e[t])){let a=e[t];t++,(a==="<"||a===">"||a==="!"||a==="=")&&t<o&&e[t]==="="&&(a+="=",t++),n.push({type:"operator",value:a});continue}n.push({type:"text",value:e[t]}),t++}return n.map(a=>`<span class="${`sql-${a.type}`}">${he(a.value)}</span>`).join("")}function he(e){return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function Te(e){return e?e.split(`
`).length:1}function ee(e){let n=Te(e);return Array.from({length:n},(t,o)=>o+1).join(`
`)}document.addEventListener("DOMContentLoaded",()=>{let{theme:e}=l.get();document.documentElement.classList.toggle("dark",e==="dark"),m(),l.subscribe(()=>{let n=l.get(),t=c("#status-text");t&&(t.textContent=n.statusText);let o=c("#status-icon");o&&(o.className=`w-2 h-2 rounded-full ${n.queryRunning?"bg-amber-400 animate-pulse":n.activeConnectionId?"bg-emerald-400":"bg-gray-600"}`)})});function m(){Ee(),Re(),Ae(),_e(),h()}function Ee(){let e=c("#sidebar");A(e);let n=l.get(),{connections:t,activeConnectionId:o,tables:a,currentTable:r}=n;e.innerHTML=`
  ...
  `,document.addEventListener("contextmenu",u=>{let d=c("#table-context-menu");d&&d.remove()});let s=c("#sidebar-body");if(!t.length&&!o){s.innerHTML=`
    ...
    `,c("#btn-new-conn")?.addEventListener("click",ae),h(s);return}let i="";for(let u of t){let d=u.id===o;if(i+=`
    ...
    `,d&&a.length){i+='<div class="ml-2 pl-2 border-l border-gray-800/60 mt-0.5">',i+=`<div class="flex items-center justify-between px-2 py-1 text-[10px] text-gray-600 uppercase tracking-wider font-medium">
        ...
        `}i+="</div>"}}s.innerHTML=i,h(s),v(s,".conn-item","click",async(u,d)=>{let p=d.dataset.id,b=t.find(f=>f.id===p);if(b){if(o===p){await V(),m();return}l.setKey("statusText",`Connecting to ${b.name}...`),m();try{await _(b),m()}catch(f){l.setKey("statusText",`Error: ${f.message}`),m()}}}),v(s,".btn-del-conn","click",(u,d)=>{u.stopPropagation(),confirm("Delete this connection?")&&(w.deleteConnection(d.dataset.id),m())}),v(s,".table-item","click",async(u,d)=>{if(u.target.closest(".btn-table-actions"))return;let p=d.dataset.table;l.get().currentTable!==p&&(await D(p),m())}),v(s,".btn-table-actions","click",(u,d)=>{u.stopPropagation(),ve(d,d.dataset.table)}),c("#btn-new-table")?.addEventListener("click",()=>oe()),o||(s.innerHTML+=`
      <div class="px-3 mt-2">
    ...
    `,c("#btn-new-conn")?.addEventListener("click",ae),h(s))}function ve(e,n){let t=c("#table-context-menu");t&&t.remove();let o=e.getBoundingClientRect(),a=C("div",{id:"table-context-menu",className:"fixed z-50 bg-gray-800 border border-gray-700/60 rounded-lg shadow-xl py-1 text-xs min-w-[140px]",style:{left:o.left+"px",top:o.bottom+4+"px"}}),r=[{label:"Browse",icon:"eye",action:()=>{D(n),m()}},{label:"Add Column",icon:"columns",action:()=>F(n)},{label:"Rename Table",icon:"edit-3",action:()=>Me(n)},{type:"divider"},{label:"Duplicate Schema",icon:"copy",action:()=>we(n)},{label:"Drop Table",icon:"trash-2",className:"text-red-400 hover:bg-red-500/10",action:()=>se(`Drop table "${n}"? This cannot be undone.`,()=>Le(n))}];for(let s of r){if(s.type==="divider"){a.appendChild(C("div",{className:"h-px bg-gray-700/60 my-1"}));continue}let i=C("button",{className:`flex items-center gap-2 w-full px-3 py-1.5 text-left hover:bg-gray-700/50 transition-colors ${s.className||"text-gray-300"}`,onClick:()=>{a.remove(),s.action()}});i.innerHTML=`<i data-lucide="${s.icon}" class="w-3 h-3"></i> ${s.label}`,a.appendChild(i)}document.body.appendChild(a),h(a),setTimeout(()=>{let s=i=>{!a.contains(i.target)&&i.target!==e&&(a.remove(),document.removeEventListener("click",s))};document.addEventListener("click",s)},0)}async function we(e){let n=M();if(!n)return;let t=e+"_copy",a=n.getTableInfo(e).columns.map(s=>({name:s.name,type:s.type,primaryKey:s.primaryKey,notNull:s.notNull,defaultValue:s.defaultValue}));await B(t,a);let r=n.getTableData(e,{limit:99999});for(let s of r.rows)await K(t,s);l.setKey("statusText",`Table "${e}" duplicated as "${t}"`),Ne(),m()}async function Le(e){await W(e),m()}function Ne(){let e=M();e&&l.setKey("tables",e.getTables())}function Re(){let e=c("#tab-bar");A(e);let{tabs:n,activeTabId:t}=l.get();if(!n.length){e.innerHTML='<div class="flex items-center px-3 text-[11px] text-gray-600">No tabs</div>';return}for(let o of n){let a=o.id===t,r=C("div",{className:`tab-item flex items-center gap-1.5 px-3 py-1.5 text-xs cursor-pointer border-r border-gray-800/60 select-none whitespace-nowrap transition-all ${a?"text-blue-400 bg-blue-500/5":"text-gray-500 hover:text-gray-300 hover:bg-gray-800/30"}`,dataset:{tabId:o.id}});r.innerHTML=`<i data-lucide="${o.type==="editor"?"terminal":"table"}" class="w-3 h-3"></i>`,r.appendChild(C("span",{className:"truncate max-w-[120px]"},o.name));let s=C("button",{className:"flex items-center justify-center w-3.5 h-3.5 rounded hover:bg-gray-700/60 transition-all ml-0.5",dataset:{tabClose:o.id}});s.innerHTML='<i data-lucide="x" class="w-2.5 h-2.5"></i>',r.appendChild(s),e.appendChild(r)}h(e),v(e,".tab-item","click",(o,a)=>{let r=parseInt(a.dataset.tabId);isNaN(r)||(n.find(i=>i.id===r)?.type==="editor"&&(l.setKey("currentTable",null),l.setKey("currentTableData",null),l.setKey("currentTableInfo",null)),w.setActiveTab(r),m())}),v(e,"[data-tab-close]","click",(o,a)=>{o.stopPropagation();let r=parseInt(a.dataset.tabClose);isNaN(r)||(w.closeTab(r),m())})}function Ae(){let e=c("#content-area");A(e);let n=l.get(),{tabs:t,activeTabId:o}=n;if(n.currentTable&&n.currentTableData){Se(e,n);return}if(!t.length||!o){e.innerHTML=`
    ...
    `,c("#btn-new-query")?.addEventListener("click",()=>{l.setKey("currentTable",null),l.setKey("currentTableData",null),l.setKey("currentTableInfo",null),w.addTab("editor"),m()}),h(e);return}let a=t.find(r=>r.id===o);a?.type==="editor"&&Ie(e,a,n)}function Ie(e,n){let t=l.get();e.innerHTML=`
  ...
  `,h(e);let o=c("#editor-input"),a=c("#editor-highlight"),r=c("#editor-gutter"),s=c("#editor-status");function i(){let d=o.value;a.innerHTML=Z(d)+`
`.repeat(Math.max(1,(d.match(/\n/g)||"").length+1)),r.textContent=ee(d),w.updateTabSQL(n.id,d)}o.addEventListener("input",i),o.addEventListener("scroll",()=>{a.scrollTop=o.scrollTop,a.scrollLeft=o.scrollLeft,r.scrollTop=o.scrollTop}),o.addEventListener("keydown",d=>{if(d.key==="Tab"){d.preventDefault();let p=o.selectionStart;o.value=o.value.substring(0,p)+"  "+o.value.substring(o.selectionEnd),o.selectionStart=o.selectionEnd=p+2,i()}(d.ctrlKey||d.metaKey)&&d.key==="Enter"&&(d.preventDefault(),ne())}),i(),setTimeout(()=>o.focus(),50),c("#btn-run")?.addEventListener("click",ne),te(t);let u=l.subscribe(()=>{te(l.get()),Ce(c("#results-panel"))},!1)}function te(e){let n=c("#editor-status");n&&(e.queryRunning?n.innerHTML='<i data-lucide="loader-circle" class="w-3 h-3 animate-spin inline-block mr-1"></i> Running...':e.queryError?n.innerHTML=`<i data-lucide="alert-circle" class="w-3 h-3 inline-block mr-1"></i> ${e.queryError}`:e.results?n.textContent=`${e.results.rows.length} rows in ${e.results.duration}ms`:n.textContent="Ready",h(n?.parentElement))}function Ce(e){if(!e)return;let n=l.get();if(n.queryRunning){e.innerHTML='<div class="flex items-center justify-center h-full gap-2 text-sm text-gray-500"><i data-lucide="loader-circle" class="w-4 h-4 animate-spin"></i> Running...</div>',h(e);return}if(n.queryError){e.innerHTML=`<div class="flex items-start gap-2 p-4 text-sm text-red-400"><i data-lucide="alert-circle" class="w-4 h-4 mt-0.5 shrink-0"></i> ${n.queryError}</div>`,h(e);return}if(!n.results?.columns?.length){e.innerHTML='<div class="flex flex-col items-center justify-center h-full gap-3 text-gray-600"><i data-lucide="arrow-up-right" class="w-8 h-8 opacity-30"></i><span class="text-xs">Run a query to see results</span></div>',h(e);return}e.innerHTML=re(n.results,!0),h(e)}async function ne(){let e=c("#editor-input")?.value?.trim();if(!(!e||!l.get().activeConnectionId))try{await Y(e)}catch{}}function Se(e,n){let t=n.currentTableData,o=n.currentTableInfo,a=n.currentTable;if(!t){e.innerHTML='<div class="flex items-center justify-center h-full text-gray-600 text-sm">Loading...</div>';return}let r=o?.columns?.find(i=>i.primaryKey)?.name||null,s=o?.columns?.map((i,u)=>`<div class="flex items-center gap-2 px-3 py-1.5 text-xs border-b border-gray-800/30 hover:bg-gray-800/20 group">
    ...
    <div class="flex flex-col h-full">
      ...
      <div class="flex flex-1 overflow-hidden">
        <div class="flex-1 flex flex-col overflow-hidden">
          ${re({columns:t.columns,rows:t.rows},!0,r,n.tableFilters,n.tableSort)}
        </div>
  ...
  `,h(e),c("#btn-query-table")?.addEventListener("click",()=>{w.addTab("editor",{connectionId:l.get().activeConnectionId,name:`Query: ${a}`,sql:`SELECT * FROM "${a}" LIMIT 100`}),m()}),v(e,".btn-add-col-inline","click",()=>F(a)),v(e,".btn-add-row-inline","click",()=>Ue(a,o)),c("#btn-add-col-panel")?.addEventListener("click",()=>F(a)),v(e,".btn-drop-col",async(i,u)=>{let d=u.dataset.col;se(`Drop column "${d}" from "${a}"?`,async()=>{await J(a,d),m()})}),Oe(e,a,r,t),$e(e,a,r),ke(e,a),De(e,a)}function Oe(e,n,t){v(e,".result-table td[data-col]","dblclick",(r,s)=>{if(s.querySelector("input, select, textarea"))return;let i=s.dataset.col,d=s.closest("tr")?.dataset?.pkVal;!t||!d||i===t||o(s,i,d,n)});function o(r,s,i,u){let p=r.closest("tr")?.closest("tbody"),b=r.querySelector(".text-gray-600.italic"),f=b?"":r.textContent;r.innerHTML=`<input class="cell-edit-input w-full bg-gray-800 border border-blue-500/50 rounded px-1 py-0.5 text-xs text-gray-200 outline-none" value="${E(f)}" />`;let T=r.querySelector("input");T.focus(),T.select();function N(){let L=T.value.trim(),R=L===""?null:L,$={};$[s]=R,r.innerHTML=R===null?'<span class="text-gray-600 italic">NULL</span>':E(R),r.title=R??"",X(u,$,t,i).then(Ke=>{l.setKey("recordCount",(l.get().recordCount||0)+1)}).catch(()=>{r.innerHTML=E(f||"NULL")})}T.addEventListener("blur",N),T.addEventListener("keydown",x=>{x.key==="Enter"?(x.preventDefault(),T.blur(),a(r,"down")):x.key==="Tab"&&!x.shiftKey?(x.preventDefault(),T.blur(),a(r,"right")):x.key==="Tab"&&x.shiftKey?(x.preventDefault(),T.blur(),a(r,"left")):x.key==="Escape"&&(x.preventDefault(),r.innerHTML=b?'<span class="text-gray-600 italic">NULL</span>':E(f))})}function a(r,s){let i=r.closest("tr");if(!i?.closest("tbody")||!i)return;let d=[...i.querySelectorAll("td[data-col]")],p=d.indexOf(r),b=null;if(s==="right"&&p<d.length-1)b=d[p+1];else if(s==="left"&&p>0)b=d[p-1];else if(s==="down"||s==="right"&&p>=d.length-1){let f=i.nextElementSibling;if(f&&f.tagName==="TR"&&!f.classList.contains("btn-add-row-inline")){let T=[...f.querySelectorAll("td[data-col]")];b=T[Math.min(p,T.length-1)]}}if(b&&!b.querySelector("input")){let f=b.dataset.col,T=b.closest("tr")?.dataset?.pkVal;t&&T&&f!==t&&o(b,f,T,n)}}}function $e(e,n,t){v(e,".btn-del-row",async(o,a)=>{let r=a.dataset.pkVal;!t||!r||confirm("Delete this row?")&&(await q(n,t,r),m())})}function ke(e,n){let t={};v(e,".filter-input","input",(o,a)=>{let r=a.dataset.filterCol,s=a.value;clearTimeout(t[r]),t[r]=setTimeout(()=>{let u={...l.get().tableFilters};s?u[r]=s:delete u[r],w.setTableFilters(u),S().then(()=>m())},250)}),v(e,".filter-input","keydown",(o,a)=>{if(o.key==="Enter"){o.preventDefault(),clearTimeout(t[a.dataset.filterCol]);let s={...l.get().tableFilters},i=a.value;i?s[a.dataset.filterCol]=i:delete s[a.dataset.filterCol],w.setTableFilters(s),S().then(()=>m())}if(o.key==="Escape"){a.value="",a.blur(),clearTimeout(t[a.dataset.filterCol]);let s={...l.get().tableFilters};delete s[a.dataset.filterCol],w.setTableFilters(s),S().then(()=>m())}}),v(e,".btn-clear-filters","click",()=>{w.setTableFilters({}),S().then(()=>m())})}function De(e,n){v(e,".sort-th","click",(t,o)=>{let a=o.dataset.sortCol;if(t.target.closest(".btn-add-col-inline"))return;let s=l.get().tableSort,i=null;!s||s.column!==a?i={column:a,direction:"asc"}:s.direction==="asc"?i={column:a,direction:"desc"}:i=null,w.setTableSort(i),S().then(()=>m())})}function re(e,n,t,o,a){let r=e.columns||[],s=e.rows||[];o=o||{},a=a||null;let i="";if(n){let d=Object.values(o).filter(p=>p&&p!=="").length;i+=`<div class="flex items-center gap-2 px-3 py-1 border-b border-gray-800/60 bg-gray-900/20 shrink-0 text-[11px]">
    ...
    </div>`}let u=r.length+(t?1:0);i+='<div class="flex-1 overflow-auto"><table class="result-table">',i+="<thead>",i+=`<tr class="sort-row">${r.map(d=>{let p=d.name||d,b=a&&a.column===p,f=b?a.direction==="asc"?"&#9650;":"&#9660;":"";return`<th class="sort-th cursor-pointer select-none hover:text-gray-200 transition-colors" data-sort-col="${p}">${p} <span class="sort-arrow text-[10px] ${b?"text-blue-400":"text-transparent"}">${f||"&#9650;"}</span></th>`}).join("")}<th class="w-9 px-1"><button class="btn-add-col-inline flex items-center justify-center w-full h-full p-0.5 rounded hover:bg-blue-500/20 hover:text-blue-400 transition-colors text-gray-600" title="Add column"><i data-lucide="plus" class="w-3.5 h-3.5"></i></button></th>${t?'<th class="w-8"></th>':""}</tr>`,i+=`<tr class="filter-row">${r.map(d=>{let p=d.name||d,b=o[p]||"";return`<td class="filter-td px-1 py-1"><input class="filter-input w-full text-[11px] px-1.5 py-1 rounded border border-gray-700/50 bg-gray-800/50 text-gray-300 outline-none placeholder-gray-600 focus:border-blue-500/40 focus:bg-gray-800 transition-all" data-filter-col="${p}" value="${E(b)}" placeholder="filter" /></td>`}).join("")}<td class="px-1"></td>${t?'<td class="w-8"></td>':""}</tr>`,i+="</thead>",i+="<tbody>";for(let d=0;d<s.length;d++){let p=s[d],b=t?p[t]:null;i+=`<tr${t?` data-pk-col="${t}" data-pk-val="${b!=null?E(String(b)):""}"`:""} data-row-idx="${d}">`;for(let f of r){let T=f.name||f,N=p[T],x;N==null?x='<span class="text-gray-600 italic">NULL</span>':typeof N=="object"?x=`<span title="${E(String(N))}">${E(JSON.stringify(N))}</span>`:x=E(String(N)),i+=`<td data-col="${T}" title="${E(String(N??""))}" class="cursor-pointer hover:bg-blue-500/5 transition-colors">${x}</td>`}i+='<td class="text-center add-cell"></td>',t&&(i+=`<td class="text-center"><button class="btn-del-row p-0.5 rounded hover:bg-red-500/20 hover:text-red-400 text-gray-600 transition-colors" data-pk-val="${b!=null?E(String(b)):""}" title="Delete row"><i data-lucide="trash-2" class="w-3 h-3"></i></button></td>`),i+="</tr>"}return i+=`<tr class="btn-add-row-inline cursor-pointer hover:bg-blue-500/5 transition-colors"><td colspan="${u+1}" class="text-center py-2 text-gray-600 hover:text-blue-400 text-xs"><span class="flex items-center justify-center gap-1"><i data-lucide="plus" class="w-3.5 h-3.5"></i> Add Row</span></td></tr>`,i+="</tbody></table></div>",i}function Ue(e,n){if(!n?.columns)return;let t=c("#modal-overlay");t.classList.remove("hidden");let o=c("#modal-content");A(o);let a=n.columns.find(i=>i.primaryKey),r=n.columns.filter(i=>!i.primaryKey||i.defaultValue===null),s="";for(let i of r){let u=i.notNull&&i.defaultValue==null?"required":"";s+=`
      <div>
  ...
  `,h(o),c("#modal-close").addEventListener("click",g),c("#modal-cancel").addEventListener("click",g),t.addEventListener("click",i=>{i.target===t&&g()}),document.addEventListener("keydown",function i(u){u.key==="Escape"&&(g(),document.removeEventListener("keydown",i))}),c("#add-row-form").addEventListener("submit",async i=>{i.preventDefault();let u={};for(let d of U(".input-add-row")){let p=d.value.trim();p===""&&!d.hasAttribute("required")?u[d.name]=null:u[d.name]=p}await K(e,u),g(),m()}),setTimeout(()=>c(".input-add-row")?.focus(),100)}function oe(){let e=c("#modal-overlay");e.classList.remove("hidden");let n=c("#modal-content");A(n),n.innerHTML=`
  ...
  `,h(n),c("#modal-close").addEventListener("click",g),c("#modal-cancel").addEventListener("click",g),e.addEventListener("click",t=>{t.target===e&&g()}),document.addEventListener("keydown",function t(o){o.key==="Escape"&&(g(),document.removeEventListener("keydown",t))}),c("#ct-add-col").addEventListener("click",()=>{let t=c("#ct-columns"),o=document.createElement("div");o.className="ct-col-row flex items-center gap-2 p-2 rounded-lg bg-gray-800/50 border border-gray-700/50",o.innerHTML=`
      ...
      <label class="flex items-center gap-1 text-[10px] text-gray-500 whitespace-nowrap"><input type="checkbox" class="ct-col-nn"> NN</label>
      <button type="button" class="ct-remove-col p-1 rounded hover:bg-red-500/20 hover:text-red-400 text-gray-600 transition-all"><i data-lucide="x" class="w-3 h-3"></i></button>
    `,t.appendChild(o),h(o)}),v(c("#ct-columns"),".ct-remove-col","click",(t,o)=>{let a=o.closest(".ct-col-row");c("#ct-columns").children.length>1&&a.remove()}),c("#create-table-form").addEventListener("submit",async t=>{t.preventDefault();let o=c("#ct-name").value.trim();if(!o)return;let a=[];for(let r of U(".ct-col-row")){let s=r.querySelector(".ct-col-name").value.trim();s&&a.push({name:s,type:r.querySelector(".ct-col-type").value,primaryKey:r.querySelector(".ct-col-pk").checked,autoIncrement:r.querySelector(".ct-col-ai").checked,notNull:r.querySelector(".ct-col-nn").checked})}a.length&&(await B(o,a),g(),m())}),setTimeout(()=>c("#ct-name")?.focus(),100)}function F(e){let n=c("#modal-overlay");n.classList.remove("hidden");let t=c("#modal-content");A(t),t.innerHTML=`
  ...
  `,h(t),c("#modal-close").addEventListener("click",g),c("#modal-cancel").addEventListener("click",g),n.addEventListener("click",o=>{o.target===n&&g()}),document.addEventListener("keydown",function o(a){a.key==="Escape"&&(g(),document.removeEventListener("keydown",o))}),c("#add-col-form").addEventListener("submit",async o=>{o.preventDefault();let a=c("#ac-name").value.trim(),r=c("#ac-type").value,s=c("#ac-default").value.trim()||null,i=c("#ac-notnull").checked;a&&(await Q(e,{name:a,type:r,defaultValue:s,notNull:i}),g(),m())}),setTimeout(()=>c("#ac-name")?.focus(),100)}function Me(e){let n=c("#modal-overlay");n.classList.remove("hidden");let t=c("#modal-content");A(t),t.innerHTML=`
  ...
  `,h(o);let a=()=>g();c("#modal-close").addEventListener("click",a),c("#modal-cancel").addEventListener("click",a),t.addEventListener("click",r=>{r.target===t&&a()}),document.addEventListener("keydown",function r(s){s.key==="Escape"&&(a(),document.removeEventListener("keydown",r))}),c("#modal-confirm").addEventListener("click",()=>{g(),n()})}function _e(){let e=c("#footer"),n=l.get();e.innerHTML=`
  ...
  `,c("#btn-new-tab")?.addEventListener("click",()=>{w.addTab("editor"),m()}),c("#btn-new-table-footer")?.addEventListener("click",()=>oe()),c("#btn-theme")?.addEventListener("click",w.toggleTheme),h(e)}function ae(e){let n=c("#modal-overlay");n.classList.remove("hidden");let t=c("#modal-content");A(t);let o=e?.database&&e.database!==":memory:";t.innerHTML=`
  ...
  `,h(t),c("#modal-close").addEventListener("click",g),c("#modal-cancel").addEventListener("click",g),n.addEventListener("click",a=>{a.target===n&&g()}),document.addEventListener("keydown",function a(r){r.key==="Escape"&&(g(),document.removeEventListener("keydown",a))}),U('input[name="conn-storage"]').forEach(a=>{a.addEventListener("change",()=>{c("#db-name-group").classList.toggle("hidden",c('input[name="conn-storage"]:checked')?.value==="memory")})}),c("#conn-form").addEventListener("submit",async a=>{a.preventDefault();let r=c("#conn-name").value.trim();if(!r)return;let s=c('input[name="conn-storage"]:checked')?.value,i=c("#conn-db")?.value?.trim()||r,u=s==="persist"?i:":memory:",d=c("#conn-seed")?.checked!==!1,p=w.addConnection({...e||{},name:r,database:u,seed:d});g(),l.setKey("statusText",`Connecting to ${r}...`),m();try{let b=l.get().connections.find(f=>f.id===p);b&&await _(b),m()}catch(b){l.setKey("statusText",`Error: ${b.message}`),m()}}),setTimeout(()=>c("#conn-name")?.focus(),100)}function g(){c("#modal-overlay").classList.add("hidden")}window.exportJSON=function(){let e=l.get(),n=e.results||(e.currentTableData?{columns:e.currentTableData.columns,rows:e.currentTableData.rows}:null);if(!n)return;let t=n.columns.map(a=>a.name||a),o=n.rows.map(a=>{let r={};return t.forEach(s=>r[s]=a[s]),r});ie(JSON.stringify(o,null,2),"results.json","application/json")};window.exportCSV=function(){let e=l.get(),n=e.results||(e.currentTableData?{columns:e.currentTableData.columns,rows:e.currentTableData.rows}:null);if(!n)return;let t=n.columns.map(r=>r.name||r),o=r=>{let s=r==null?"":String(r);return s.includes(",")||s.includes('"')||s.includes(`
`)?'"'+s.replace(/"/g,'""')+'"':s},a=[t.map(o).join(","),...n.rows.map(r=>t.map(s=>o(r[s])).join(","))];ie(a.join(`
`),"results.csv","text/csv")};function ie(e,n,t){let o=new Blob([e],{type:t}),a=document.createElement("a");a.href=URL.createObjectURL(o),a.download=n,a.click(),URL.revokeObjectURL(a.href)}
